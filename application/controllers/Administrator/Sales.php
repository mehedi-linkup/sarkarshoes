...
class Sales extends CI_Controller {
    ...
    public function addSales(){
        ...
        try{
            ...
            if ($data->customer->Customer_Type == 'G' && $data->sales->paid < $data->sales->total) {
                $res = ['success' => false, 'message' => 'Cash customer can not due..!'];
            ...
            if(empty($customerId) && $data->customer->Customer_Type != 'G') {
                $customerCheck = $this->db->query("select * from tbl_customer where Customer_Mobile = ? and Customer_brunchid = ?", [$data->customer->Customer_Mobile, $this->session->userdata("BRANCHid")])->row();

                if($customerCheck) {
                    $customerId = $customerCheck->Customer_SlNo;
                }else{
                    $customer = (array)$data->customer;
                    unset($customer['Customer_SlNo']);
                    unset($customer['display_name']);
                    
                    $customer['Customer_Name'] = empty($data->customer->Customer_Name) ? null : $data->customer->Customer_Name;
                    $customer['Customer_Address'] = empty($data->customer->Customer_Address) ? null : $data->customer->Customer_Address;
            ...
            $sales = array(
                ...
                'SalseCustomer_IDNo' => $data->customer->Customer_Type != 'G' ? $customerId : null,
                'customerType' => $data->customer->Customer_Type == 'G' ? $data->customer->Customer_Type : null,
                'customerName' => $data->customer->Customer_Type == 'G' ? $data->customer->Customer_Name : null,
                'customerPhone' => $data->customer->Customer_Type == 'G' ? $data->customer->Customer_Mobile : null,
                'customerAddress' => $data->customer->Customer_Type == 'G' ? $data->customer->Customer_Address : null,
                'employee_id' => $data->sales->employeeId,
                'SaleMaster_SaleDate' => $data->sales->salesDate,
                'saleMethod' => $data->sales->saleMethod ?? '',
                ...
                'cashPaid' => $data->sales->cashPaid ?? 0,
                'returnAmount' => $data->sales->returnAmount ?? 0,
    ...
    public function getSales(){
        ...
        $sales = $this->db->query("
            select 
            concat_ws(' - ', sm.SaleMaster_InvoiceNo, c.Customer_Name) as invoice_text,
            sm.*,
            c.Customer_Code,
            c.Customer_Name,
            c.Customer_Mobile,
...
//    }
    ...
    public function getColorWithStock() 
    {
        $data = json_decode($this->input->raw_input_stream);

        $clauses = "";
        if (!empty($data->colorId)) {
            $clauses .= " and c.color_SiNo = '$data->colorId'";
        }
        
        $stock = $this->mt->getColorWiseStock($clauses, $data->productId);

        echo json_encode($stock);
    }

    public function getSizeWithStock()
    {
        $data = json_decode($this->input->raw_input_stream);
        $clauses = "";
        if (!empty($data->sizeId)) {
            $clauses .= " and s.id = '$data->sizeId'";
        }

        $stock = $this->mt->getSizeWiseStock($clauses, $data->colorId, $data->productId);
    ...
    public function posSales($serviceOrProduct = 'product')
    {
        ...
        $data['invoice'] = $invoice;
        $data['saleMethod'] = 'pos';
        $data['content'] = $this->load->view('Administrator/sales/pos_sales', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
