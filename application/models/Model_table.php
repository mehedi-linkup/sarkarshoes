...
class Model_Table extends CI_Model{
    ...
    public function getColorWiseStock($clauses = '', $productId) 
    {
        $branchId = $this->session->userdata('BRANCHid');

        $stock = $this->db->query("
            select
                pd.color_id as color_SiNo,
                pd.Product_IDNo,
                c.color_name,
                (select ifnull(sum(pd2.PurchaseDetails_TotalQuantity), 0) from tbl_purchasedetails pd2
                where pd2.Status = 'a'
                and pd2.PurchaseDetails_branchID = '$branchId'
                and pd2.Product_IDNo = pd.Product_IDNo
                and pd2.color_id = pd.color_id
                and pd2.size_id is not null
                and pd2.color_id is not null
                ) as purchase_qty,
                (select ifnull(sum(sd.SaleDetails_TotalQuantity), 0) from tbl_saledetails sd
                where sd.Status = 'a'
                and sd.SaleDetails_BranchId = '$branchId'
                and sd.Product_IDNo = pd.Product_IDNo
                and sd.color_id = pd.color_id
                and sd.size_id is not null
                and sd.color_id is not null
                ) as sale_qty,
                (select purchase_qty - sale_qty) as stock
            from tbl_purchasedetails pd
            left join tbl_color c on c.color_SiNo = pd.color_id
            where pd.Status = 'a'
            and pd.size_id is not null
            and pd.color_id is not null
            and pd.PurchaseDetails_branchID = '$branchId'
            and pd.Product_IDNo = '$productId'
            $clauses
    ...
    public function getSizeWiseStock($clauses = '', $colorId, $productId) {
        $branchId = $this->session->userdata('BRANCHid');
        ...
        $stock = $this->db->query("
            select
                ...
                and pd2.PurchaseDetails_branchID = '$branchId'
                and pd2.Product_IDNo = pd.Product_IDNo
                and pd2.color_id = '$colorId'
                and pd2.size_id = pd.size_id
                and pd2.color_id is not null
                and pd2.size_id is not null
                ) as purchase_qty,
                (select ifnull(sum(sd.SaleDetails_TotalQuantity), 0) from tbl_saledetails sd
                ...
                and sd.SaleDetails_BranchId = '$branchId'
                and sd.Product_IDNo = pd.Product_IDNo
                and sd.color_id = '$colorId'
                and sd.size_id = pd.size_id
                and sd.color_id is not null
                and sd.size_id is not null
                ) as sale_qty,
                (select purchase_qty - sale_qty) as stock
            from tbl_purchasedetails pd
            left join tbl_size s on s.id = pd.size_id
            where pd.Status = 'a'
            and pd.color_id is not null
            and pd.size_id is not null
            and pd.PurchaseDetails_branchID = '$branchId'
