						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									...
									<div class="col-xs-8">
										<v-select v-if="!isProductFilterByBarcode" v-bind:options="products" id="product" v-model="selectedProduct" label="display_text" v-on:input="productOnChange"></v-select>
										<input v-else type="text" v-model="barcodeValue" placeholder="Barcode" ref="barcode" class="form-control">
									</div>
									<div class="col-xs-1" style="padding: 0;">
										...
										<a href="#" @click.prevent="changeProductFilter" class="btn btn-xs btn-danger" style="height: 25px; border: 0; width: 27px; margin-left: -10px;" target="_blank" title="Add New Product"><i class="fa fa-barcode" aria-hidden="true" style="margin-top: 5px;"></i></a>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-xs-9">
										<input type="number" step="0.01" id="quantity" placeholder="Qty" class="form-control" v-model="selectedProduct.quantity" v-on:input="productTotal" autocomplete="off" required />
									</div>
								</div>
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					<thead>
						<tr class="">
							...
							<th style="width:40%;color:#000;">Product Name</th>
							<th style="width:8%;color:#000;">Qty</th>
							<th style="width:12%;color:#000;">Rate</th>
							<th style="width:20%;color:#000;">Amount</th>
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td> <input type="number" style="margin: 0; height: 25px;" step="1" min="1" @input="editQuantity(product)" class="form-control" v-model="product.quantity"></td>
							<td> <input type="number" style="margin: 0; height: 25px;" step="0.01" min="1" @input="editSaleRate(product)" class="form-control" v-model="product.salesRate"></td>
							<td>{{ product.total }}</td>
							...
							<div class="table-responsive">
								<table style="color:#000;margin-bottom: 0px;border-collapse: collapse;">
									...
									<tr style="display: none;">
										<td>
											<div class="form-group">
												...
												<div class="col-xs-12">
													<input type="number" id="paid" class="form-control" v-model="sales.paid" v-on:input="calculateTotal" disabled />
												</div>
											</div>
										</td>
									</tr>
									...
									<tr>
										<td>
											<div class="form-group">
												<label class="col-xs-12 control-label no-padding-right">Cash Paid</label>
												<div class="col-xs-12">
													<input type="number" id="cashPaid" class="form-control" v-model="sales.cashPaid" v-on:input="returnTotal" />
												</div>
											</div>
										</td>
									</tr>
									...
									<tr>
										<td>
											<div class="form-group">
												...
												<div class="col-xs-12">
													<input type="number" id="returnAmount" class="form-control" v-model="sales.returnAmount" v-on:input="returnTotal" disabled />
												</div>
											</div>
										</td>
									</tr>
		...
		data() {
			return {
				sales: {
					...
					saleMethod: '<?php echo $saleMethod; ?>',
					salesType: 'retail',
					salesFrom: '',
					salesDate: '',
					customerId: '',
		...
		methods: {
			...
			async addToCart() {
				...
				if (this.isProductFilterByBarcode && this.barcodeValue != '') {
					let filteredProducts = this.products.filter(p => p.Product_Code == this.barcodeValue);

					if (filteredProducts.length) {
						selectedProduct = filteredProducts[0];

						if ((selectedProduct.Product_SlNo != '' || selectedProduct.Product_SlNo != 0) && this
				...
				if (!selectedProduct.total) {
					selectedProduct.total = (quantity * parseFloat(selectedProduct.Product_SellingPrice))
						.toFixed(2);
				}
				...
				if (this.selectedProduct.quantity > 0) {
					...
					if (cartInd > -1) {
						if ((this.cart[cartInd].quantity + product.quantity) <= this.productStock) {
							this.cart[cartInd].quantity += product.quantity;
							this.cart[cartInd].total = this.cart[cartInd].salesRate * this.cart[cartInd]
								.quantity;
							this.clearProduct();
							this.calculateTotal();
						} else {
				...
				if (!this.isProductFilterByBarcode && !this.barcodeValue != '') {
					this.$refs.product.$refs.search.focus();
				}
			...
			async editQuantity(product) {
				...
				if (+product.quantity > +productStock && this.sales.isService == 'false') {
					alert('Stock unavailable');
					product.quantity = productStock;
				}

				product.total = parseFloat(product.quantity * product.salesRate).toFixed(2);
				this.calculateTotal();
			},

			editSaleRate(product) {
			...
			returnTotal() {
				...
				if (parseFloat(this.sales.cashPaid) >= parseFloat(this.sales.total)) {
					this.sales.returnAmount = parseFloat(this.sales.cashPaid) - parseFloat(this.sales.total);
				}
			...
			async saveSales() {
				...
				if(this.sales.cashPaid == 0 || +this.sales.cashPaid < +this.sales.total) {
					alert('Cash paid amount less then total amount!');
					return;
				}
			...
			async getSales() {
				await axios.post('/get_sales', {
					...
					this.sales.saleMethod = sales.saleMethod;
					...
					if (sales.customerType != 'G') {
						...
						this.selectedCustomer = {
							...
							display_name: 'Cash Customer',
							Customer_Mobile: sales.customerPhone,
							Customer_Address: sales.customerAddress,
